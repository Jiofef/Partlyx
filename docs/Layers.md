...
Core - Represents the data model, but enhanced with logic for organizing the hierarchy of objects and with stronger encapsulation, so that objects are more difficult to break from outside Core. Also provides general readonly information for application, such as option schemes.
...
UI.Avalonia - The visual part of the application. Responsible only for presentation, with the minimal code required to connect ViewModels or initialize dependency injection.